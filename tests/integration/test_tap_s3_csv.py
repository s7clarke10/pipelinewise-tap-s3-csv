...
class TestTapS3Csv(unittest.TestCase):
            ...
            "streams": [
                {
                    ...
                    "schema": {
                        "properties": {
                            ...
                            "is_pensioneer": {"type": ["null", "string"]},
                    ...
                    "metadata": [
                        ...
                        {
                            "breadcrumb": ["properties", "_sdc_source_bucket"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
                            "breadcrumb": ["properties", "_sdc_source_file"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
                            "breadcrumb": ["properties", "_sdc_source_lineno"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
                            "breadcrumb": ["properties", "birth_date"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
                        ...
                        {
                            "breadcrumb": ["properties", "ip_address"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
                            "breadcrumb": ["properties", "is_pensioneer"],
                            "metadata": {"inclusion": "available"},
                        },
                        {
    ...
    def test_discovery(self):
        ...
        self.assertEqual(
            self.expected_catalog["streams"][0]["stream"],
            catalog["streams"][0]["stream"],
        )
        self.assertEqual(
            self.expected_catalog["streams"][0]["tap_stream_id"],
            catalog["streams"][0]["tap_stream_id"],
        )

        self.assertDictEqual(
            self.expected_catalog["streams"][0]["schema"],
            catalog["streams"][0]["schema"],
        )
        self.assertListEqual(
            self.expected_catalog["streams"][0]["metadata"],
            # sort metadata to have have a fix order of breadcrumbs to make
            # assertion pass
            sorted(
                catalog["streams"][0]["metadata"], key=lambda d: d["breadcrumb"]
            ),
        )
    ...
    def test_sync(self):
        ...
        self.assertEqual(
            catalog["streams"][0]["schema"].keys(), lines[1]["schema"].keys()
        )
        ...
        self.assertEqual(
            100, sum(1 for line in lines if line["type"] == "RECORD")
        )
